...
public class Calc {
    private int number_one, number_two;     //числа в выражении
    ...
    public String result(String exp) throws CalcException{
        ...
        if (parse.isNumeric(expItems.get(0)) && parse.isNumeric(expItems.get(2))){      //---проверяем, что оба числа арабские
            number_one = Integer.parseInt(expItems.get(0));
            number_two = Integer.parseInt(expItems.get(2));
            isRomanExp = false;
        } else if (parse.isRoman(expItems.get(0)) && parse.isRoman(expItems.get(2))){   //---проверяем, что оба числа римские
            number_one = parse.romeToArabConvert(expItems.get(0));
            number_two = parse.romeToArabConvert(expItems.get(2));
            isRomanExp = true;
        } else {    //--- числа не соответствуют
            throw new CalcException("ОШИБКА. Используются одновременно разные системы счисления");
        }



        //---проверка чисел, должны быть от 1 до 10 включительно
        if (!(number_one>=1 && number_one<=10)){
            throw new CalcException("ОШИБКА. Число #1 должно быть от 1 до 10 или от I до X включительно");
        }

        if (!(number_two>=1 && number_two<=10)){
        ...
        //--- получаем результат
        int res = calcExp(number_one, operator, number_two);
